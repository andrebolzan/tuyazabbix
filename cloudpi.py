...
def get_device_status_cloud(device_id, device_type):
    ...
    if result.get('success'):
        if device_type == 'sensor':
            online = c.getconnectstatus(device_id) # verifica se esta online e retornar | .get('online', 0)

            temp = next(item['value'] for item in result['result'] if item['code'] == 'va_temperature')
            humi = next(item['value'] for item in result['result'] if item['code'] == 'va_humidity')
            batt = next(item['value'] for item in result['result'] if item['code'] == 'battery_state')

            output = f"online;{online};temp;{temp};humi;{humi};batt;{batt};api;{result.get('success')}"
            print(output)
        elif device_type == 'smoke':
            online = c.getconnectstatus(device_id) # verifica se esta online e retornar
...
def get_device_status_local(device_id, ipv5, key_local):
...
if __name__ == "__main__":
    ...
    parser.add_argument('device_type', type=str, help='Tipo de dispositivo (ex: sensor)')
    ...
    if args.mode == 'cloud':
        ...
        if args.ipv5 and args.key_local:
            # Chamar a função para consulta local
            get_device_status_local(args.device_id, args.ipv5, args.key_local)
        else:
